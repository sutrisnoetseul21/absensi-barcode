...
class UpdateDataSiswaAction extends Action
{
    public static function getDefaultName(): ?string
    {
        return 'update_data_siswa';
    }
    ...
    protected function setUp(): void
    {
        ...
        $this
            ->label('Update Data Siswa (Excel)')
            ->icon('heroicon-o-arrow-up-on-square')
            ->color('warning')
            ->modalHeading('Update Massal Data Siswa')
            ->modalDescription('Perbarui informasi siswa (biodata, nomor HP, orang tua, dsb) secara massal menggunakan file Excel.')
            ->modalWidth('4xl')
            ->form([
                // ── Tombol Download Data Siswa Aktif ─────────────────────────
                \Filament\Forms\Components\Placeholder::make('download_data_box')
                    ->label('')
                    ->content(new \Illuminate\Support\HtmlString('
                        <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px 16px; background-color: #FFFBEB; border: 1px solid #FDE68A; border-radius: 8px; margin-bottom: 4px;">
                            <div>
                                <span style="font-weight: 700; color: #92400E; display: block; font-size: 0.9rem;">Unduh data siswa terkini untuk diedit</span>
                                <span style="font-size: 0.8rem; color: #B45309;">File Excel berisi seluruh data siswa aktif lengkap beserta kolom ID pengenal.</span>
                            </div>
                            <a href="' . route('admin.siswa.download-data') . '" target="_blank" style="display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; background-color: #D97706; color: white; border-radius: 6px; font-weight: 600; font-size: 0.825rem; text-decoration: none; box-shadow: 0 1px 2px rgba(0,0,0,0.05); white-space: nowrap;">
                ...
                \Filament\Forms\Components\FileUpload::make('file')
                    ->label('Pilih file Excel yang telah diperbarui (.xlsx)')
                    ->disk('local')
                    ->directory('imports')
                    ->acceptedFileTypes([
                    ...
                    ->required()
                    ->live()
                    ->helperText('Pastikan Anda tidak mengubah atau menghapus kolom pertama (ID).'),
                        ...
                        try {
                            ...
                            if (strtolower(trim((string)($headers[0] ?? ''))) !== 'id') {
                                return new \Illuminate\Support\HtmlString(
                                    ...
                                    . '⚠️ Berkas tidak valid. Kolom pertama harus berupa ID. Pastikan Anda mengunggah file hasil dari tombol <strong>Download Data (Update)</strong>.'
                                    . '</div>'
                                );
                            }
                            ...
                            foreach (array_slice($rows, 0, 100) as $row) {
                                ...
                                $tgl     = trim((string)($row[5] ?? ''));
                                $jk      = trim((string)($row[6] ?? ''));
                                ...
                                $tableRows .= '<td style="padding: 8px 12px; color: #374151; border-right: 1px solid #E5E7EB; text-align: center;">' . htmlspecialchars($jk ?: '-') . '</td>';
                                $tableRows .= '<td style="padding: 8px 12px; color: #166534; font-weight: 600;">' . $this->badgeHtml("Siap Diperbarui", '#166534', '#D1FAE5') . '</td>';
                                $tableRows .= '</tr>';
                            }
                            ...
                            $html .= '<th style="padding: 10px 12px; font-weight: 600; color: #374151; border-bottom: 2px solid #E5E7EB; border-right: 1px solid #E5E7EB; text-align: left;">NIS</th>';
                            ...
                            $html .= '<th style="padding: 10px 12px; font-weight: 600; color: #374151; border-bottom: 2px solid #E5E7EB; border-right: 1px solid #E5E7EB; text-align: center;">L/P</th>';
                            $html .= '<th style="padding: 10px 12px; font-weight: 600; color: #374151; border-bottom: 2px solid #E5E7EB; text-align: left;">Status Preview</th>';
                            $html .= '</tr></thead>';
            ...
            ->action(function (array $data) {
                ...
                $body = "✅ {$summary['berhasil']} siswa berhasil diperbarui | ℹ️ {$summary['skip']} data dilewati (tidak ada perubahan atau tidak valid).";
