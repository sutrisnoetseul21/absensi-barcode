<div class="space-y-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <div class="flex items-center gap-2">
                    <h2 class="text-base font-extrabold text-slate-900">Sarana & Prasarana</h2>
                    <span class="text-[11px] font-bold text-violet-700 bg-violet-50 px-2 py-0.5 rounded-full border border-violet-100">{{ $sarpras->total() }} Fasilitas</span>
                </div>
                <p class="text-xs text-slate-400">Kelola dan atur fasilitas sarpras sekolah</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <div class="relative flex-1 sm:w-60">
                <input type="text" wire:model.live.debounce.400ms="search" placeholder="Cari fasilitas..." class="w-full pl-8 pr-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-slate-900 text-xs focus:ring-2 focus:ring-violet-400 focus:bg-white outline-none">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        ...
        <div class="bg-white rounded-xl shadow-xs border border-slate-200/80 hover:border-violet-300 hover:shadow-md transition-all p-3.5 flex flex-col justify-between group relative">
            ...
            {{-- Center: Icon --}}
            <div class="w-12 h-12 mx-auto my-1.5 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:scale-110 group-hover:bg-violet-50 transition-all duration-200">
                @if($item->icon)
                    <i class="{{ $item->icon }} {{ $item->color ?? 'text-slate-400' }} text-2xl"></i>
                @else
            ...
            <div class="text-center mt-1">
                <h3 class="font-bold text-slate-800 text-xs line-clamp-1 group-hover:text-violet-600 transition-colors" title="{{ $item->nama_fasilitas }}">
                    {{ $item->nama_fasilitas }}
                </h3>
                <p class="text-[10px] text-slate-400 line-clamp-1 mt-0.5" title="{{ $item->deskripsi }}">
        ...
        <div class="col-span-2 sm:col-span-3 md:col-span-4 lg:col-span-5 py-12 text-center text-slate-400">
            ...
            <p class="text-xs">Belum ada data sarana prasarana</p>
        </div>
        @endforelse
    </div>

    @if($sarpras->hasPages())
        <div class="bg-white rounded-xl shadow-xs border border-slate-200 px-4 py-3">{{ $sarpras->links() }}</div>
    @endif
