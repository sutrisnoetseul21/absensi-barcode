<div class="min-h-screen bg-slate-100 flex items-center justify-center relative overflow-hidden font-sans" 
    ...
    <div x-show="!isActive" 
        ...
        <h1 class="text-4xl font-extrabold text-white tracking-wider">Sentuh Layar Untuk Mengaktifkan Layanan Sirkulasi</h1>
    ...
    <div class="relative w-full max-w-5xl mx-4 my-8">
        ...
        <div class="absolute -top-12 left-0 right-0 flex justify-between items-center px-2">
            <a href="/portal-perpustakaan" class="text-slate-600 hover:text-slate-900 font-bold flex items-center bg-white/70 px-4 py-2 rounded-full backdrop-blur-sm transition text-xs shadow-xs">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                Kembali ke Dashboard
            </a>
            <button @click="resetToPeminjam()" x-show="scanState === 'BUKU'" class="text-white bg-rose-600 hover:bg-rose-700 font-bold text-xs px-4 py-2 rounded-full shadow transition flex items-center">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                Batalkan & Reset Anggota
            </button>
        </div>
        ...
        <div class="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white overflow-hidden flex flex-col md:flex-row min-h-[540px]">
            ...
            <div class="w-full md:w-5/12 bg-gradient-to-b from-indigo-700 to-indigo-900 text-white p-6 lg:p-8 flex flex-col relative transition-all duration-300">
                ...
                <!-- Member Active Info (Step 2) -->
                <div x-show="scanState === 'BUKU'" style="display: none;" class="mb-6 space-y-4">
                    <div class="p-4 bg-white/10 rounded-2xl border border-white/20 backdrop-blur-md flex items-center gap-3">
                        <div class="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center text-white flex-shrink-0 font-extrabold text-lg">
                        ...
                        <div class="flex-grow overflow-hidden">
                            ...
                            <h4 class="text-base font-extrabold text-white truncate" x-text="peminjamInfo.name"></h4>
                            <p class="text-xs text-indigo-100 truncate" x-text="peminjamInfo.sub_info"></p>
                        </div>
                    </div>

                    <!-- Active Loans List Box -->
                    <div class="space-y-2">
                ...
                <div class="mt-auto pt-4 space-y-4 border-t border-white/10">
                    ...
                    <div class="flex items-center space-x-3 transition-opacity" :class="scanState === 'BUKU' ? 'opacity-100' : 'opacity-40'">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs"
                             :class="scanState === 'BUKU' ? 'bg-emerald-500 text-white ring-2 ring-emerald-300' : 'bg-white/20 text-white'">
                            2
                        </div>
                        <div class="text-left">
                ...
                <div x-show="isLoading" class="absolute inset-0 bg-black/40 flex flex-col items-center justify-center backdrop-blur-sm z-20">
                    ...
                    <span class="font-bold text-sm text-white tracking-wider">Memproses Data...</span>
                </div>
            </div>
            ...
            <div class="w-full md:w-7/12 p-6 lg:p-8 flex flex-col relative bg-slate-50/50">
                ...
                <!-- Feedback Alert Bar -->
                <div x-show="feedbackState !== 'idle'" style="display: none;" class="mb-4 p-4 rounded-2xl border transition-all flex items-center gap-3 shadow-xs"
                     :class="{
                ...
                <!-- STATE 1: STEP 1 SCAN ANGGOTA -->
                <div x-show="scanState === 'PEMINJAM' && !isCameraActive" class="flex-grow flex flex-col items-center justify-center text-center p-6">
                    <div class="w-28 h-28 mx-auto rounded-3xl bg-indigo-50 border border-indigo-100 flex items-center justify-center mb-5 shadow-inner animate-pulse">
                        <svg class="w-14 h-14 text-brand-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2"></path></svg>
                    </div>
                    <h3 class="text-2xl font-extrabold text-slate-800 mb-2">Silakan Scan Kartu Anggota</h3>
                    <p class="text-slate-500 text-sm max-w-md">Dekatkan stiker barcode pada kartu presensi siswa atau guru ke scanner.</p>
                </div>

                <!-- STATE 2: STEP 2 SCAN BUKU (DRAFT CART) -->
                <div x-show="scanState === 'BUKU'" style="display: none;" class="flex-grow flex flex-col space-y-4">
                    ...
                    <div class="flex-grow flex flex-col bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
                        <div class="px-4 py-3 bg-slate-100/70 border-b border-slate-200 flex justify-between items-center text-xs font-extrabold text-slate-700">
                            ...
                            <span class="px-2 py-0.5 bg-brand-primary/10 text-brand-primary rounded-full text-[11px]" x-text="draftCart.length + ' Buku Ter-scan'"></span>
                        </div>
                        ...
                        <div class="flex-grow overflow-y-auto max-h-64 p-3 space-y-2">
                            <template x-for="(item, index) in draftCart" :key="item.kode_eksemplar">
                                <div class="p-3 rounded-xl border border-slate-200 bg-slate-50/50 hover:bg-white flex items-center justify-between gap-3 transition-all">
                                    ...
                                    <!-- Book Info & Action Badge -->
                                    <div class="flex-grow overflow-hidden space-y-1">
                                        <div class="flex items-center gap-2">
                                            <!-- Action Badge -->
                                            <span x-show="item.action_type === 'PINJAM'" class="px-2 py-0.5 rounded-md text-[10px] font-black bg-emerald-100 text-emerald-800 border border-emerald-200">
                                                PINJAM BARU
                                            </span>
                                            <span x-show="item.action_type === 'KEMBALI'" class="px-2 py-0.5 rounded-md text-[10px] font-black bg-sky-100 text-sky-800 border border-sky-200">
                                                PENGEMBALIAN
                                            </span>
                                            <span x-show="item.action_type === 'PERPANJANG'" class="px-2 py-0.5 rounded-md text-[10px] font-black bg-purple-100 text-purple-800 border border-purple-200">
                                                PERPANJANG +7 HARI
                                            </span>
                                    ...
                                    <!-- Action Switch & Delete Button -->
                                    <div class="flex items-center gap-2 flex-shrink-0">
                                        <!-- Toggle Return vs Extend if borrowed -->
                                        ...
                                        <!-- Hapus Button (Trash Icon) -->
                                        <button type="button" @click="removeFromCart(index)" class="p-1.5 bg-rose-50 text-rose-600 hover:bg-rose-100 border border-rose-200 rounded-lg transition-all" title="Hapus dari daftar (Batal Scan)">
                            ...
                            <div x-show="draftCart.length === 0" class="flex flex-col items-center justify-center py-8 text-center text-slate-400">
                                ...
                                <p class="text-xs font-bold text-slate-500">Keranjang Transaksi Kosong</p>
                                <p class="text-[11px] text-slate-400 mt-0.5">Scan barcode buku untuk memasukkan ke dalam daftar.</p>
                            </div>
                        </div>

                        <!-- Footer Submit Button Toolbar -->
                        <div class="p-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-3">
                            <button type="button" @click="clearCart()" x-show="draftCart.length > 0" class="px-3 py-2 text-rose-600 hover:bg-rose-50 border border-rose-200 rounded-xl text-xs font-bold transition-all">
                                Kosongkan
                            </button>
        ...
        <!-- Footer Input Buffer Status Bar -->
        <div class="mt-4 bg-white/70 border border-slate-200 rounded-xl p-3.5 flex flex-col md:flex-row justify-between items-center gap-3 text-xs text-slate-500 backdrop-blur-sm shadow-xs">
            <div class="flex items-center gap-3 w-full md:w-auto justify-between md:justify-start">
                ...
                <div class="flex items-center space-x-2">
                    <span class="w-2.5 h-2.5 rounded-full" :class="isActive ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'"></span>
                    <span class="hidden md:inline" x-text="isActive ? 'Sistem Aktif & Siap Scan' : 'Menunggu Aktivasi'"></span>
                </div>
            </div>
    ...
    <script>
        function sirkulasiKioskData() {
            return {
                ...
                scanState: 'PEMINJAM', // 'PEMINJAM' atau 'BUKU'
                    ...
                    try {
                        ...
                        if (this.scanState === 'PEMINJAM') {
                            ...
                            if (!response.ok) {
                                ...
                                const errData = await response.json().catch(() => ({ message: `Terjadi kesalahan (Status ${response.status})` }));
                ...
                playAudio(type) {
                    try {
                        const idMap = { success: 'audio-success', error: 'audio-error', network: 'audio-network' };
