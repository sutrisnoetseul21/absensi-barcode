<div class="min-h-screen flex flex-col lg:flex-row bg-slate-900 lg:bg-white font-jakarta relative overflow-hidden">
    ...
    <div class="absolute inset-0 z-0 lg:hidden">
        ...
        <div class="absolute top-1/4 -left-20 w-72 h-72 bg-emerald-500 rounded-full mix-blend-screen filter blur-[80px] opacity-30 animate-blob"></div>
        <div class="absolute bottom-1/4 -right-20 w-72 h-72 bg-teal-500 rounded-full mix-blend-screen filter blur-[80px] opacity-30 animate-blob animation-delay-2000"></div>
    </div>
    ...
    <div class="relative hidden lg:flex lg:w-1/2 bg-slate-900 overflow-hidden items-center justify-center p-12 z-10">
        <div class="absolute inset-0 z-0">
            ...
            <!-- Decorative Blobs -->
            <div class="absolute top-1/4 -left-20 w-72 h-72 bg-emerald-500 rounded-full mix-blend-screen filter blur-[80px] opacity-40 animate-blob"></div>
            <div class="absolute bottom-1/4 -right-20 w-72 h-72 bg-teal-500 rounded-full mix-blend-screen filter blur-[80px] opacity-40 animate-blob animation-delay-2000"></div>
        </div>
        ...
        <div class="relative z-10 w-full max-w-lg text-left">
            <div class="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p class="text-emerald-200 text-sm">Berbasis Barcode</p>
                </div>
            </div>
            <h1 class="text-4xl lg:text-5xl font-extrabold text-white mb-6 leading-tight">
                Selamat Datang di <br><span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-300 drop-shadow-sm">Portal Siswa</span>
            </h1>
            <p class="text-lg text-slate-300 leading-relaxed mb-8 max-w-md">
                Akses dashboard akademik Anda, pantau riwayat kehadiran harian, dan tingkatkan kedisiplinan belajar.
            </p>
    ...
    <div class="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-20 xl:px-24 bg-transparent lg:bg-white relative z-10">
        <!-- Back to Home -->
        <a href="/" class="absolute top-6 right-6 lg:top-8 lg:right-8 flex items-center gap-2 text-sm font-semibold text-slate-300 lg:text-slate-500 hover:text-emerald-400 lg:hover:text-emerald-600 transition-colors">
        ...
        <div class="mx-auto w-full max-w-sm bg-white/10 lg:bg-transparent backdrop-blur-xl lg:backdrop-blur-none border border-white/20 lg:border-none rounded-3xl lg:rounded-none p-6 sm:p-8 lg:p-0 shadow-2xl lg:shadow-none">
            ...
            <div class="lg:hidden text-center mb-8">
                ...
                <h2 class="text-2xl font-extrabold text-white tracking-tight drop-shadow-md">Portal Siswa</h2>
                <p class="text-sm text-emerald-100 mt-1 drop-shadow-sm">Sistem Presensi Berbasis Barcode</p>
            </div>

            <div>
                <h2 class="mt-6 text-3xl font-extrabold text-slate-900 tracking-tight hidden lg:block">Masuk ke Akun</h2>
                <p class="mt-2 text-sm text-slate-500 hidden lg:block">Silakan masukkan NISN dan password Anda.</p>
            </div>

            <div class="mt-8 lg:mt-10">
                <form wire:submit.prevent="login" class="space-y-5">
                    <div>
                        <label for="nisn" class="block text-sm font-semibold text-white lg:text-slate-700 mb-1.5 drop-shadow-sm lg:drop-shadow-none">NISN Siswa</label>
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-slate-400 group-focus-within:text-emerald-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                            </div>
                            <input wire:model="nisn" id="nisn" type="text" required class="block w-full pl-11 pr-3 py-3 border border-slate-200 lg:border-slate-200 rounded-xl leading-5 bg-white/90 lg:bg-slate-50 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 lg:focus:ring-emerald-500/20 focus:border-emerald-500 focus:bg-white transition-all duration-200 sm:text-sm shadow-sm" placeholder="Masukkan NISN Anda">
                        </div>
                        @error('nisn') <p class="mt-2 text-xs text-rose-300 lg:text-rose-500 font-medium drop-shadow-sm lg:drop-shadow-none">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <input wire:model="remember" id="remember" type="checkbox" class="h-4 w-4 text-emerald-600 focus:ring-emerald-500 border-slate-300 rounded cursor-pointer transition-colors">
                    ...
                    <div class="pt-2">
                        <button type="submit" class="w-full flex justify-center items-center gap-2 py-3.5 px-4 border border-transparent rounded-xl shadow-lg shadow-emerald-900/50 lg:shadow-emerald-500/30 text-sm font-bold text-white bg-gradient-to-r from-emerald-500 to-teal-400 lg:from-emerald-600 lg:to-teal-500 hover:from-emerald-400 hover:to-teal-300 lg:hover:from-emerald-500 lg:hover:to-teal-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transition-all duration-300 transform hover:-translate-y-0.5">
                            Masuk ke Dashboard
