    ...
    <div class="paper-sheet" @if(!$loop->last) style="page-break-after: always; break-after: page;" @endif>
        ...
        <table class="identitas-table">
            ...
            <tr>
                ...
                <td class="sep">:</td>
                <td class="val">{{ $st->siswa->nis ?? '—' }} / {{ $st->siswa->nisn ?? '—' }}</td>
        ...
        @if($periode === 'tahunan')
            ...
            <div style="font-weight: bold; font-size: 9.5pt; margin: 4px 0 3px 0;">
                A. Semester 1 (Ganjil: Juli – Desember {{ $tahunAjaran->start_year ?? date('Y') }}) &bull; Total: {{ $st->semester1Jurnals->count() }} Sesi
            </div>
            <table class="data-table">
                ...
                <tbody>
                    @forelse($st->semester1Jurnals as $idx => $jurnal)
                    <tr>
            ...
            <div style="font-weight: bold; font-size: 9.5pt; margin: 8px 0 3px 0;">
                B. Semester 2 (Genap: Januari – Juni {{ $tahunAjaran->end_year ?? (date('Y') + 1) }}) &bull; Total: {{ $st->semester2Jurnals->count() }} Sesi
            </div>
            <table class="data-table">
                <thead>
                    <tr>
                ...
                <tbody>
                    @forelse($st->semester2Jurnals as $idx => $jurnal)
                    <tr>
                        <td class="center">{{ $idx + 1 }}</td>
                        <td class="center">
                            {{ \Carbon\Carbon::parse($jurnal->tanggal_waktu)->translatedFormat('d/m/Y') }}<br>
        ...
        @else
            ...
            <table class="data-table">
                ...
                <tbody>
                    @forelse($st->jurnals as $idx => $jurnal)
                    <tr>
                        <td class="center">{{ $idx + 1 }}</td>
                        <td class="center">
                            {{ \Carbon\Carbon::parse($jurnal->tanggal_waktu)->translatedFormat('d/m/Y') }}<br>
        ...
        <table class="data-table">
            ...
            <tbody>
                @forelse($st->konsultasis as $kIdx => $k)
                <tr>
        ...
        <table class="pilar-summary-table">
            ...
            <tbody>
                <tr>
                    <td><strong>{{ $st->distribusiPilar['Akademik'] }}</strong> Sesi</td>
                    <td><strong>{{ $st->distribusiPilar['Karakter & Kedisiplinan'] }}</strong> Sesi</td>
                    <td><strong>{{ $st->distribusiPilar['Minat & Bakat / Ekskul'] }}</strong> Sesi</td>
                    <td><strong>{{ $st->distribusiPilar['Sosial & Psikologis'] }}</strong> Sesi</td>
                    <td style="background-color: #f1f5f9;"><strong>{{ $st->jurnals->count() }}</strong> Sesi</td>
                </tr>
            </tbody>
        </table>
            ...
            <table class="ttd-table">
                <tr>
                    <td>
                        ...
                        <div class="ttd-nip">Wali dari {{ $st->siswa->name }}</div>
                    </td>
                    <td>
